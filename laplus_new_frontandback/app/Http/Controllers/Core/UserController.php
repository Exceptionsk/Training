...
class UserController extends Controller
{
    ...
    public function pending()
    {
      if (Auth::guard('User')->check()) {
          ...
          $cur_time   = Carbon::now();
          return view('core.user.pending')->with('users', $users)->with('cur_time',
              $cur_time);
      }
      return redirect('/backend/login');
    }
    ...
    {
      if (Auth::guard('User')->check()) {
          $users      = $this->userRepository->getApprovedUsers();
          $cur_time   = Carbon::now();
          return view('core.user.approved')->with('users', $users)->with('cur_time',
              $cur_time);
      }
      return redirect('/backend/login');
    }
    ...
    {
      if (Auth::guard('User')->check()) {
          $users      = $this->userRepository->approve($id);
          $cur_time   = Carbon::now();
          // return view('core.user.pending')->with('users', $users)->with('cur_time',
          //     $cur_time);
      }
